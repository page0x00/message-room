...
更新 GitHub Pages 会更新前端，**不会自动修改 Supabase**。本轮没有线上数据库管理员权限，以下操作需要项目所有者执行。旧 `messages` 不删除、不清空。
...
- 原图 OCR 在浏览器完成。首次从固定 Tesseract CDN 下载 WASM 与中英文模型，模型下载失败可粘贴系统相册识别文字。草稿可改来源/正文/日期/选择项，确认后逐条发送；媒体占位可手动补充，不能自动恢复截图中的原图/语音/视频文件。
...
GitHub Pages 部署 **main / 根目录**，必须保留整个仓库静态文件结构。前端版本为 `2.2.0`；JS/CSS 入口带版本参数，Service Worker 只处理通知，不缓存页面、消息或媒体。直接访问 [正式页面](https://page0x00.github.io/message-room/)。
